...
(function(module){
  ...
  var Blog = {
    ...
    generateIndexRss: function(options, categories, posts, dest, callback) {
      var perPage = options.rss.count || 5;
      var template = options.rss.template;
      var selectedPosts = posts.slice(0, perPage);
      var output = '/rss.xml';
      var templateData = {
        posts: selectedPosts,
        categories: Object.keys(categories)
      ...
      Blog.generatePage(options.templateEngine, template, templateData, dest + output, callback);
    },
    generateCategory: function(options, category, posts, dest, callback) {
      ...
      async.times(totalPages, function(n, done) {
        ...
        var selectedPosts = posts.slice(perPage * n, perPage * page);
        var output = '/' + options.categories.paginationUrl.replace('XX', page).replace('CC', category);
        var templateData = {
          posts: selectedPosts,
          category: category,
          page: page,
    ...
    generateCategoryRss: function(options, category, posts, dest, callback) {
      var perPage = options.rss.count || 5;
      var template = options.rss.templateCategory;
      var selectedPosts = posts.slice(0, perPage);
      var output = '/' + category + '.xml';
      var templateData = {
        posts: selectedPosts,
        category: category
      };
    ...
    generateCategoriesRss: function(options, categories, dest, callback) {
      var categoryList = Object.keys(categories);

      async.eachLimit(categoryList, 5, function(category, done) {
        var categoryPosts = categories[category];
        Blog.generateCategoryRss(options, category, categoryPosts, dest, done);
      }, function(err) {
        callback();
      });
    }
